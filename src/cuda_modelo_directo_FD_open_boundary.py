...
class WaveModeloSolucion:
    ...
    def solver(self, speed_square, speed_square_grad):
        ...
        # Initialize arrays
        w0 = cp.zeros((batch_size, n_points), dtype="float32")
        w1 = cp.zeros((batch_size, n_points), dtype="float32")
        ...
        for t_index in range(n_time_steps-1 ):
            ...
            # # Update w1 with boundary conditions
            # w1[:, : self.soporte.x_nodes] = out[:,
            #                                     : self.soporte.x_nodes, t_index - 1]
            # w1[:, self.soporte.x_nodes:] = out[:,
            #                                    self.soporte.x_nodes:, t_index - 1]
            
            # w1[:, 0] = self.soporte.source_left[t_index]

            # # Compute next step
            # u_next, v_next = self.step(w1, w0, speed_square, speed_square_grad)

            # # Store results
            # out[:, :, t_index] = cp.concatenate((u_next, v_next), axis=1)

            # # Update w0 for the next iteration
            # w0[:] = w1
        return out
