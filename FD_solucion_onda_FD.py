...
from cuda_modelo_directo_FD_open_boundary import WaveModeloSolucion
...
def FD_FD(lambda_nodes):
    ...
    dx=soporte.x_dx
    # nodes=5
    Gx, Lx = FD_FD_operadores(nodes, dx)
...
def FD_FD_operadores(nodes, dx):
    # Operadores derivadas para diferencias finitas hacia adelante
    Gx= cp.diag(np.tile(-1.,nodes),k=0,)+cp.diag(np.tile(1.,nodes-1),k=1)
    Gx[-1,-2:]=cp.array([1.,-1])
    Gx=Gx/dx
